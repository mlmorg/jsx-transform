...
function visitNode(traverse, object, path, state) {
	...
	var ignoreAttributes = !attributes.length && options.ignoreEmptyAttributes;
	...
	attributes.forEach(function(attr, index) {
		...
		var previousAttr = attributes[index - 1];
		var name = attr.name.name;

		utils.catchup(attr.range[0], state, trimLeft);

		if (previousAttr) {
			var previousLine = previousAttr.loc.start.line;
			var currentLine = attr.loc.start.line;
	...
	if (children.length) {
		...
		if (lastRenderableIndex !== undefined && !ignoreAttributes) {
			utils.append(', ', state);
		}
...
function renderXJSLiteral(object, isLast, state, start, end) {
	...
	lines.forEach(function (line, index) {
		...
		if (trimmedLine || isLastNonEmptyLine) {
			var safeContent = trimmedLine.replace(/'/g, '\\\'');
			var additionalSpace = !isLastNonEmptyLine ? ' ' : '';
			var concat = !isLastNonEmptyLine ? ',' : '';
			var lineString = "'" + safeContent + additionalSpace + "'" + concat;
			utils.append(lineString, state);
